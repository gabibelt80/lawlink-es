...
export function matterAssociationFilter(userId: string, role?: UserRole) {
  if (role === "ADMIN" || role === "PRINCIPAL_LAWYER" || role === "FINANCE") return {};
  return { OR: [{ ownerId: userId }, { members: { some: { userId } } }] };
}
...
export async function assertCanHandleMatter(
  ...
  if (matter.ownerId === userId) return;
  if (matter.members.some((m) => m.userId === userId)) return;
  throw new Error("Caso no existe o sin permiso de procesamiento");
}
...
export function assertManagerOrRole(role: UserRole, ...allowed: UserRole[]): void {
