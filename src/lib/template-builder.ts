...
async function buildT2(): Promise<Buffer> {
  return pack([
    ...
    kvTable([
      ...
      ["y被告人关系", ""],
      ["联系电话", "{{client.phone}}"],
      ["被告人Nombre y apellido", "{{opposing.name}}"],
...
async function buildT3(): Promise<Buffer> {
  return pack([
    ...
    blank(),
    body(
      "本所y本所Abogado在接受您的委托办理 {{matter.causeText}} 一案前，依据《Abogado法》《Abogado执业行为规范》etc.相关规定，将以下法律服务风险事ítems明确告知您，请仔细阅读：",
      { indent: true }
    ),
    blank(),
    body("一、法律服务结果的不Aceptar性。法律事务的处理受Caso事实、证据、法律适用、司法裁量、对方当事人行为etc.多种因素影响，Abogado无法承诺任何Aceptar的结果。", { indent: true }),
    body("二、Caso结果不取决于代理费Monto。Abogado收费y办案投入相关，y诉讼结果无对应关系。", { indent: true }),
    body("三、证据材料的真实性责任。委托人提供的证据材料须真实、合法。如因证据虚假或瑕疵导致不利后果，由委托人自行承担。", { indent: true }),
    body("四、诉讼时效y举证Plazo。委托人应当在法律规定的诉讼时效内主张权利，在举证Plazo内EnviarVer todos证据，Vencido可能丧失相应权利。", { indent: true }),
    body("五、判决的执行风险。即使获得胜诉判决，因对方履行能力etc.Motivo，仍可能存在执行不能或执行不到位的风险。", { indent: true }),
    body("六、和解y调解的可能性。Abogado将根据Caso情况评估和解、调解方案，是否接受由委托人最终决定。", { indent: true }),
    body("七、其他事ítems。", { indent: true }),
    blank(),
...
async function buildT4(): Promise<Buffer> {
  return pack([
    ...
    blank(),
    body("第一条 委托事ítemsy代理权限", { bold: true }),
    body("甲方委托乙方指派Abogado就 {{matter.causeText}} 一案为甲方提供法律服务。代理权限为：________________(一般代理 / 特别代理：包括代为承认、放弃、变更诉讼请求，代为和解，代为提起反诉或上诉etc.)。", { indent: true }),
    blank(),
    body("第二条 委托代理事ítems的范围", { bold: true }),
    body("(一审 / 二审 / 再审 / 仲裁 / 执行)", { indent: true }),
    blank(),
    body("第三条 Abogado费y支付方式", { bold: true }),
    body("代理费Monto：人民币________pesos(大写：________________pesos整)。", { indent: true }),
    body("支付方式：________________。", { indent: true }),
    blank(),
    body("第四条 其他Gastos", { bold: true }),
    body("Caso办理过程中产生的诉讼费、Preservación费、鉴定费、差旅费etc.，由甲方另行承担。", { indent: true }),
    blank(),
    body("第五条 双方权利义务", { bold: true }),
    body("略", { indent: true }),
    blank(),
    body("第六条 合同的解除y终止", { bold: true }),
    body("略", { indent: true }),
    blank(),
...
async function buildT5(): Promise<Buffer> {
  return pack([
    ...
    body("电话：{{firm.phone}}"),
    blank(),
    body("甲乙双方就以下事ítems签订本委托代理合同：", { indent: true }),
    blank(),
    body("第一条 委托事ítems", { bold: true }),
    ...
    body("自本合同签订之日起至本案代理事ítems处理完毕(取得生效法律文书或双方书面终止)。", { indent: true }),
    blank(),
    body("第五条 保密条款", { bold: true }),
    body("乙方对甲方提供的MaterialyCaso信息负有保密义务。", { indent: true }),
    blank(),
...
async function buildT6(): Promise<Buffer> {
  return pack([
    ...
    blank(),
    body("现委托上列受委托人在我y {{opposing.name}} {{matter.causeText}} 一案中，作为我的诉讼代理人。", { indent: true }),
    blank(),
...
async function buildT7(): Promise<Buffer> {
  return pack([
    ...
    body("诉讼标的Monto：{{matter.claimAmount}}", { bold: true }),
    blank(),
    body("诉讼请求：", { bold: true }),
    body("1. ________________；", { indent: true }),
    body("2. ________________；", { indent: true }),
    body("3. 本案诉讼费、Preservación费etc.由被告承担。", { indent: true }),
    blank(),
...
async function buildT8(): Promise<Buffer> {
  return pack([
    ...
    body("________________________________________________________________________", { indent: true }),
    blank(),
    body("综上，请求贵院依法Rechazar被答辩人的诉讼请求，以维护答辩人合法权益。", { indent: true }),
    blank(),
    blank(),
    body("此致"),
    body("{{proceeding.court}}", { bold: true }),
...
async function buildT9(): Promise<Buffer> {
  return pack([
    ...
    kvTable([
      ...
      ["Causa", "{{matter.causeText}}"],
      ["Cerrar caso方式", "{{archive.closedReasonCN}}"],
...
const T10_VARS = [
  ...
  "archive.archivedAtCN",
  "lawyer.name"
  // documents[] Aprobar运行时 inject，不在 detectMissing 范围
];
...
function docCatalogLoopRow(): TableRow {
  // docxtemplater 在表格循环：单pesos格内分别用 {{#documents}}...{{/documents}} 包裹会失效；
  // 标准做法是把 loop 标签放在整行外层，行 cell 内只放纯 {{var}}。这里用注释占位行 + 文档生成时手工插入循环标签。
  // 为简化，直接 build 一行 placeholders，loop 包裹Aprobar docxtemplater 的 row loop 自动识别（同一行第一个 cell 含 {{#documents}}）。
...
export const BUILTIN_TEMPLATES: BuiltInTemplate[] = [
  ...
  },
  {
    ...
    name: "PenalCaso收案登记表",
    category: "INTAKE",
    description: "PenalCaso收案信息登记。被告人羁押地点etc.关键字段。",
  ...
  },
  {
    ...
    name: "法律服务风险告知书",
    category: "INTAKE",
    description: "向委托人告知法律服务的不Aceptar性y各类风险。Abogadoy委托人签字。",
  ...
  },
  {
    ...
    name: "民事起诉状",
    category: "LITIGATION",
    description: "民事起诉状标准格式。诉讼请求y事实理由需Abogado填充。",
    applicableCategories: ["CIVIL_COMMERCIAL"],
    variables: T7_VARS,
    buildBuffer: buildT7
  },
  ...
  },
  {
    ...
    name: "卷宗封皮",
    category: "ARCHIVE",
    description: "归档时自动生成。律所标识 + Caso标题 + 归档编号 + Cerrar caso信息。Abogado勿手动渲染。",
    applicableCategories: [],
    variables: T9_VARS,
    buildBuffer: buildT9
  },
