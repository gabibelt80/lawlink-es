 ...
 * - Las imágenes usan reconocimiento visual aiVision
 * - El PDF primero intenta extracción de texto con unpdf + aiChat (bajo costo, rápido)
 * - Cuando la capa de texto del PDF está vacía (documento escaneado) retrocede: unpdf renderPageAsImage renderiza las primeras 3 páginas → reconocimiento aiVision por página → fusión de resultados
 */
...
const SYSTEM_PROMPT = `Sos un asistente de análisis de documentos legales argentinos. La imagen de abajo es una demanda / solicitud / solicitud de arbitraje.
Respondé estrictamente con el siguiente formato JSON (solo JSON, sin ninguna explicación):
{
  ...
  "claimAmount": número (en pesos argentinos, solo para monto monetario; si no es monetario, usá null),
  "claimDescription": "Resumen de la solicitud / pretensión principal",
  "court": "Nombre completo del tribunal o arbitraje competente"
}
Reglas:
- Si no se encuentra un campo, devolvé [] o null, no inventes información.
...
// Fusionar resultados de páginas escaneadas: las listas de partes se deduplican por nombre, los campos escalares toman el primer valor no vacío
...
export async function parsePleading(form: FormData): Promise<ParsedPleading> {
  ...
  try {
    ...
    // PDF: primero intenta extracción de capa de texto (menor costo)
    ...
    if (cleaned && cleaned.length >= 20) {
      const { content } = await aiChat({
        messages: [
          ...
          {
            role: "user",
            content: `A continuación el texto completo de la demanda / solicitud:\n\n${cleaned.slice(0, 12000)}`,
          },
        ],
        maxTokens: 1500,
      });
    ...
    // v0.27: Capa de texto vacía (PDF escaneado) → renderiza las primeras 3 páginas como PNG, reconocimiento visual por página y fusión
    ...
    for (let i = 1; i <= pagesToRender; i++) {
      ...
      const { content } = await aiVision({
        image: { dataUrl },
        prompt: `${SYSTEM_PROMPT}\n\n(Esta es la página ${i}/${pagesToRender} de la demanda / solicitud escaneada)`,
        maxTokens: 1500,
      });
    ...
    if (pageResults.length === 0) {
      throw new Error("Error al reconocer PDF escaneado, subí una imagen o revisá el archivo");
    }
