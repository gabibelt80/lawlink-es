  ...
  return (
    <section className="ll-surface rounded-lg border border-border p-5">
      <header className="mb-4 flex items-center gap-2">
        ...
        <h2 className="text-lg">Calculadora de tasa judicial</h2>
        <span className="ml-2 text-[10px] text-muted-foreground">
          Normativa nacional de tasas judiciales
        </span>
      </header>

      <div className="space-y-3">
        <div>
          <Label className="text-[11px]">Tipo de caso</Label>
          <RadioChips
            size="sm"
            className="mt-2"
            items={CASE_TYPES}
        ...
        {showAmount && (
          <div>
            <Label className="text-[11px]">
              {caseType === "DIVORCE" ? "Monto de división de bienes (pesos, puede ser 0)" : "Monto del objeto del litigio (pesos)"}
            </Label>
            <Input
              type="number"
              value={amountInput}
            ...
            {amount > 0 && (
              <p className="mt-1 text-[10px] text-muted-foreground">
                En letras: {numberToChinese(amount)}
              </p>
            )}
          </div>
        )}
      ...
      {result && (
        <>
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <ResultCard label="Procedimiento ordinario" value={result.fee} accent="#4F46E5" />
            <ResultCard label="Procedimiento simplificado (mitad)" value={result.feeSimplified} accent="#16a34a" />
          </div>
