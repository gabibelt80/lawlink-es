...
- `ADMIN` / `PRINCIPAL_LAWYER` 可在通讯录中查看待审核联系人，并进行通过或驳回；通过后全所展示，驳回后不展示。
...
- `User.calendarToken`（unique，URL 即凭证）；设置 → 个人信息 新增"日历订阅"卡片：复制订阅链接 / 重置（作废旧链接）
...
- 失败给所有 ADMIN 发 HIGH 站内通知（备份静默失败等于没有备份）；`BACKUP_CRON_ENABLED=false` 可关（无 pg_dump 的部署环境）
...
| 3 | 备份默认开启、失败通知 ADMIN | 自部署用户不会主动配备份；失败可感知比默认关闭更安全 |
...
1. **外部联系人审核流默认关闭**：新增 SystemSetting `workflowToggles.externalContactReview`（默认 false——新增联系人直接通过并全所可见）；律所信息页（ADMIN）"工作流开关"卡片可打开审核。原审核通道、通知、审计逻辑保留不动。
...
**v1.0 尚余**：P3 文书模板扩容（8 → 20+，内容生产工作，独立任务推进）。
